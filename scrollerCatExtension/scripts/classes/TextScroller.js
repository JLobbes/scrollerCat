    ...
    class TextScroller {
        ...
        assessRightBoundaryWords() {
            ...
            do {
                ...
                const isLastWord =
                    index >= this.wordPositionMap_Array.length - 1;
                ...
                if (isWithinAdditionZone && !isLastWord) {
                    this.visibleWords[index + 1] =
                        this.wordPostionMap_Object[index + 1]; // Adds one word to visible words
        ...
        getWordMap() {
            ...
            const hasChineseCharacters =
                /[的一是在不了有和人這中大為上個國我以要他時來用們生到作地於出就分對成會可主發年動同工也能下過子說產種面而方後多定行學法所民得經十三之進著等部度家電力裡如水化高自二理起小物現實加量都兩體制機當使點從業本去把性好應開它合還因由其些然前外天政四日那社義事平形相全表間樣與關各重新線內數正心反你明看原又麼利比或但質氣第向道命此變條只沒結解問意建月公無系軍很情者最立代想已通並提直題黨程展五果料象員革位入常文總次品式活設及管特件長求老頭基資邊流路級少圖山統接知較將組見計別她手角期根論運農指幾九區強放決西被再做客實線名傳爭設士星車勢演熱標急嚴英奇買九該失才察步隨史權馬強草廠夜皮固屬找段油查整深轉術復職覺冷具推顯候曾觀船秀課洲孤凱印藍劃藝剛注環普奔村羅貨橋額序雅層爸房編止課怕環普奔村羅貨橋額序雅層爸房編止課怕環普奔村羅貨橋額序雅層爸房編止課怕環普奔村羅貨橋額序雅層爸房編止課怕環普奔村羅貨橋額序雅層爸房編止課怕環普奔村羅貨橋額序雅層爸房編止課怕環普奔村羅貨橋額序雅層爸房編止課怕環普奔村羅貨橋額序雅層爸房編止課怕環普奔]/.test(
                    textRaw
                );
            ...
            this.wordPositionMap_Array = textProcessed.map(
                (word) => new window.Word(word, 0)
            );
            ...
            this.scrollerText.innerHTML = this.wordPositionMap_Array
                .map(
                    (wordObj, index) =>
                        `<span class="word" id="word-${index}">${wordObj.text}</span>`
                )
                .join("");
        ...
        jumpPositionTo(targetScrollPosition) {
            ...
            this.getCurrentScrollPosition();
            const amountMoved =
                targetScrollPosition - this.currentScrollPosition;
        ...
        startDrag(e) {
            ...
            this.isDown = true;
            this.cursorStartX =
                (e.pageX || e.touches[0].pageX) * this.dragMultiplier -
                this.scrollerText.offsetLeft;
            this.removeSmoothTransition();
        }
        ...
        setAutoScrollConfiguration(autoScrollConfiguration) {
            ...
            this.scrollerTextTransition =
                autoScrollConfiguration.autoScrollTransition;
        ...
        setFontSize(fontSize) {
            ...
            for (const index in currentWords) {
                if (Object.hasOwnProperty.call(currentWords, index)) {
                    ...
                    const distanceOffCenter =
                        -this.currentScrollPosition - position;
                    if (
                        Math.abs(distanceOffCenter) <
                        Math.abs(referenceWord.distanceFromCenter)
                    ) {
            ...
            const refWordNewData =
                this.wordPostionMap_Object[referenceWord.index];
        ...
        getCurrentScrollPosition() {
            this.scrollerTextLeft =
                this.scrollerText.getBoundingClientRect().left;
            this.scrollerBoxCenter =
                this.scrollerBox.getBoundingClientRect().left +
                this.scrollerBox.getBoundingClientRect().width / 2;

            this.currentScrollPosition =
                this.scrollerTextLeft - this.scrollerBoxCenter;
